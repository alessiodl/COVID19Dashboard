...
const createSlider = function(_data_){
    ...
    slider.noUiSlider.on('update', function (values, handle){
        ...
        axios.get(url+'/andamento',{ params:{ data: slider_date } }).then(function(response){
            ...
            var totale_positivi = response.data[0].totale_positivi
            var nuovi_positivi = response.data[0].variazione_totale_positivi
            var aggiornamento = response.data[0].data;
            // Update total count
            document.querySelector("#tot-contagi").innerHTML = totale_casi
            document.querySelector("#tot-positivi").innerHTML = totale_positivi
            document.querySelector("#nuovi-positivi").innerHTML = nuovi_positivi
            // Update date
